...
function setInitialData(obj) {
	itinerary_data = obj;
	//showInfo(JSON.stringify(itinerary_data));

	var sql = "select territory_id from psr";
	doSQL(sql, function(o) {
		territory_id = o[0].territory_id;
...
function events(){
	...
	$("#save_ok").click(function() {
		...
		// showMessage("alert-info", "SQL: "+sql);
		// showMessage("alert-info", "SQL Local: "+sql_local);
